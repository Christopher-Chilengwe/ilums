      ...
      <div class="table-responsive">         
        <table id="studentlogs" class="table table-striped table-bordered table-hover"  style="font-size:12px" cellspacing="0">
         ...
         <thead>
				  ...
				  <tbody>
					...
					if (isset($_POST['search'])){
					     ...
					     $res = $mydb->setQuery("SELECT * FROM tblenrollment WHERE COURSE='{$_POST['course']}' and YEARLEVEL='{$_POST['grdlvl']}' ORDER BY STUDENTNAME");
         ...
         echo '<a href="index.php?view=print&course='.$_POST['course'].'&yr='.$_POST['grdlvl'].'&flag=1" class="btn btn-primary">Print</a>';
						...
						$res = $mydb->setQuery("SELECT * FROM tblenrollment WHERE AY='{$_POST['ay']}' and SEMESTER='{$_POST['sem']}' ORDER BY STUDENTNAME");
					       $row_cnt = $mydb->num_rows();
					        if ($row_cnt > 0) {
					      	$cur = $mydb->loadResultList();
							foreach ($cur as $sy) {
